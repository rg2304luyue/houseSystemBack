...
def fetch_github_user_data_sync(code):
    """同步获取 GitHub 用户数据（供 OAuth 回调直接调用）"""
    try:
...
def fetch_github_user_data(code):
    """Celery 异步任务：获取 GitHub 用户数据（预留异步场景）"""
    return fetch_github_user_data_sync(code)

def send_email_smtp(email, verification_code, subject='邮箱验证码'):
    from flask import current_app

    sender_email = current_app.config.get('QQ_SMTP_EMAIL')
    authorization_code = current_app.config.get('QQ_SMTP_AUTH_CODE')

    if not sender_email or not authorization_code:
        raise RuntimeError("QQ_SMTP_EMAIL 或 QQ_SMTP_AUTH_CODE 未配置")
